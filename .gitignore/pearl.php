...
if(verify())
{
	...
	elsif($cmd eq "upl") { upload_test(); 	}
	elsif($cmd eq "ftp") { ftp_test(); 	}
	elsif($cmd eq "htp") { http_get(); 	}
	elsif($cmd eq "tim") { system_time(); 	}
	elsif($cmd eq "psh") { pseudo_shell(); 	}
	elsif($cmd eq "cpu") { cpu_test(); 	}
			else { no_command(); 	}
}
...
sub cpu_test
{
	...
	<B>Gives a performance index of mathematical processing power</B><BR><BR>
HTMLCODE
	# Without time::hires ( not a commonly installed module )
	# we have a problem running a test over a precise amount of time.
	# We get around this by using time(), waiting until it clicks over and then
	# running for 2 seconds. Just adding 2 seconds on without waiting would mean 
	# we could be running for between 2 and 2.99 seconds
	#
	# We only run for 2 seconds so as not to spike server usage too much
	# Results are consistent on a non-loaded server
	my $start = time();
	my $index = 0;
	...
	$lines .= <<HTMLCODE;
	Script completed <strong>$index</strong> itterations in two seconds\n<br>
	<br>Approximate benchmark guide :<br>
	<li>1800 : Non Loaded p4 3.2Ghz
	<li>1700 : Medium load production environment Dual Xeon 3.0Ghz
	<li>1500 : Medium load production environment Dual PIII 1.2Ghz
	<li>0700 : Non Loaded AMD K6 300Mhz
	<br><br><strong>REMEMBER!</strong> Index scale is non-linear.<br>
	<br><small><strong>How the test is performed</strong><br>The script starts at an index of one, and each 
	itteration this increases this by one. Every cycle, the factorial of the index is calculated. 
	This can result in very large, and very CPU instensive cycles. On the same system, an index of 500 is 
	performed four times quicker than the index of 1000, which is four times quicker than the index of 2000 .. and so on.
HTMLCODE
	my $html = html_source();
...
sub system_time
{
	...
	<TR>
	<TD VALIGN="TOP" BGCOLOR="#EEEEEE" COLSPAN=2>
	<B>Reports the current system time</B><BR><BR>
HTMLCODE
	$lines .= "System Localtime : <strong>".localtime()."</strong><br>\n";
	$lines .= "GMT Time as reported by the server : <strong>".gmtime()."</strong></td></tr></table>\n";
	my $html = html_source();
	$html =~ s/##content##/$lines/g;
	output ("text/html", $html); 
}
...
{
	my $lines = <<HTMLCODE;
	<H1>[Remote HTTP Get Test]</H1>
	<TABLE CELLSPACING=2 CELLPADDING=2 WIDTH=600>
	<TR>
	<TD VALIGN="TOP" BGCOLOR="#EEEEEE" COLSPAN=2>
	<B>Attempts to use <i>LWP::UserAgent</i> and <i>HTTP::Request</i> to retrieve a remote page.</B><BR><BR>
HTMLCODE
	...
	($url =~ /(http|https)\:\/\/.*/) or $url = "http://$url";
	...
	my $header = $ua->request($req);
		
	my $content;
	
	my $head = "Status : ".$response->status_line."\n";
	$head .= "Content-Type : ".$header->content_type."\n";
	$head .= "Content Length : ".$header->content_length."\n";
	$head .= "Last Modified : ".$header->last_modified."\n";
		
	if($response->is_success) { $content= $response->content; }
	else{$content="Retrieval Failed!";}
	...
	<FORM ACTION="$scripturl" METHOD="post" ENCTYPE="multipart/form-data">
		 Test another URL ( http or https )
		 <br><small>( Use :xx to specify another port, eg 'fasthosts.co.uk:1222')</small><br>
		 <input type="text" name="url">
		 <input type="submit" name="Submit" value="Test">
		 <INPUT TYPE="HIDDEN" NAME="cmd" VALUE="htp">
		 <INPUT TYPE="HIDDEN" NAME="key" VALUE="$key">
	</FORM>
	<strong>Retrieving $url</strong><br><br>
	<b>Page Headers</b><pre>$head</pre><br>
	<b>Page Content</b><form><textarea rows=20 cols=71>$content</textarea></form>End of retrieval<br><br>
	</td></tr></table>\n
HTMLCODE
	my $html = html_source();
	$html =~ s/##content##/$lines/g;
...
{
	my $lines = <<HTMLCODE;
	<H1>[FTP Connection Test]</H1>
	<TABLE CELLSPACING=2 CELLPADDING=2 WIDTH=600>
	<TR>
	<TD VALIGN="TOP" BGCOLOR="#EEEEEE" COLSPAN=2>
	<B>FTP Connection test</B><BR><BR>
HTMLCODE
	...
	if($action eq "test")
	{
		...
		my $pasv = $cgi->param("pasv");
		my $testfile = $rootpath.$cgi->param("file");

		$lines .= "Testing connection to '$host'<br>Username='$user'<br>File='$testfile'<br><br>\n";
		...
		if($ftp->login("$user","$pass"))
		{
			...
			{
				$ftp->pasv();
				$lines .= "Using PASSIVE mode transfer<br><br>\n";
			}
			...
			if($ftp->cwd($target))
			{
				if($ftp->put($testfile))
				{
					$lines .= "<strong>Congratulations! It appears that the file was uploaded correctly.</strong>";
				}
				else
				{
					if(-d $testfile)
					{
						$lines .= "<strong>Failed on upload.<br><i>$testfile</i><br>is a directory, not a file!.</strong>";
					}
					elsif(-e $testfile)
					{
						$lines .= "<strong>Failed on upload of<br><i>$testfile</i><br>to $host.</strong>";
					}
					else
					{
						$lines .= "<strong>Failed to upload!<br><i>$testfile</i><br>Does not exist locally!</strong>";
					}
				}
			}
			else
			{
				$lines .= "<strong>Failed on chdir to $target</strong>";
			}
		}
		else
		{
			$lines .= "<strong>Failed login. FTP Username or password invalid.</strong>";
		}
		$ftp->quit;
	}
	else
	{
		$lines .= <<HTMLCODE;
		Tests a connection from the webserver to a FTP server using NET::FTP<br> 
		It does not upload a file from your local PC to the webserver ( use the file upload test for that. )<br>
		<strong>This feature is not neccessarily supported by your webhhost!</strong>
		<form action="$scripturl" method="post">
		Username : <br><input type="text" name="user" size=50><br><br>
		Password : <br><input type="password" name="pass" size=50><br><br>
		Target Directory : <br><input type="text" name="target" size=50><br><br>
		File to upload (in relation to the webroot)<br>This must be located on the webserver, NOT your local machine:<br><small>( $rootpath )</small><br><input type="text" name="file" size=50><br><br>
		Target host ( FQDN or IP address ): <br><input type="text" name="host" size=50><br><br>
		Passive mode <input type="checkbox" name="pasv">
		<input type="hidden" name="key" value="$key">
 	...
 	if($^O =~ /linux/i)
 	{
		...
		if( $action )
		{
			if($action =~ /^cd (.*)/)
			{
				...
				$dir =~ s/\/\//\//g;
				if($result eq "Awaiting Command")
				{
					$result = "Failed to change directory to $dir/$1";
				}
				else
				{
					$result .= " to $dir";
			...
			{
				$dir =~ s/\/\//\//g;
				$result = `cd $olddir;$action 2>&1`;
			}
		}
		if($result =~ /^sh: .*(cd: .*)/)
		{
			$result = $1;
			($action =~ /^cd.*/) and $dir = $olddir;
		}
		($result) or $result="OK";
		my $shortdir = $dir;
		($dir =~ /.*\/(.*?)$/) and $shortdir= $1;
		...
		my $host = `hostname`;
		chomp $host;
		my $dnsdomain = `dnsdomainname`;
		chomp $dnsdomain;
		$result =~ s/\</\&lt\;/g;
		$result =~ s/\>/\&gt\;/g;
		$lines .= <<HTMLCODE;
		<H1>[Linux Pseudo Shell]</H1>
		<TABLE CELLSPACING=2 CELLPADDING=2 WIDTH=600>
		<TR>
		<TD VALIGN="TOP" BGCOLOR="#EEEEEE" COLSPAN=2>
		<FONT COLOR="#FF0000">WARNING</font><br>
		Making changes with this utility can be permanent and damaging. Do <FONT COLOR="#FF0000">NOT</font> use this facility unless you are <FONT COLOR="#FF0000">ABSOLUTELY</font> sure you know what you are doing.
		</td></tr></table><br>
		
		<TABLE CELLSPACING=2 CELLPADDING=2 WIDTH=600>
		<TR>
		<TD VALIGN="TOP" CLASS=system>
		<FORM METHOD="POST" ACTION="$scriptname" NAME="sysform">
		<INPUT TYPE="HIDDEN" NAME="cmd" VALUE="psh">
		<INPUT TYPE="HIDDEN" NAME="key" VALUE="$key">
		<INPUT TYPE="HIDDEN" NAME="dir" VALUE="$dir">
		[<FONT COLOR="#FF0000">User:</FONT>$user &nbsp; <FONT COLOR="#FF0000">Host:</FONT>$host]<BR>
		[<FONT COLOR="#FF0000">Current Dir:</FONT>$shortdir]<BR>
		\$ <INPUT TYPE="TEXT" NAME="action" SIZE=50 CLASS=system>
		<INPUT TYPE="SUBMIT" NAME="submit" VALUE="Go" CLASS=system>
		<HR>
		<FONT COLOR="#FF0000">executed:</FONT>$action
		<HR><FONT COLOR="#FF0000">result from server:</FONT><PRE CLASS=system>$result</PRE>
		</FORM>
		<script language="JavaScript">
		<!--
		document.sysform.action.focus();
		//--></script>
		</TD></TR>
		</TABLE>
HTMLCODE
	...
	<TD VALIGN="TOP" BGCOLOR="#EEEEEE" COLSPAN=2>
	<B>Error</B><BR><BR>
	This utility will only work on a Linux platform.
	</td></tr></table>
HTMLCODE
	}
	my $html = html_source();
...
sub upload_test
{
	...
	<TR>
	<TD VALIGN="TOP" BGCOLOR="#EEEEEE" COLSPAN=2>
	<B>File Upload test</B><BR><BR>
HTMLCODE
	my $action = $cgi->param("action");
	if($action eq "upload")
	{
		my $file=$cgi->param("file");
		my $uploadpath=$cgi->param("path");
		my $filename = $file;
		if($file)
		{
		        ($file =~ /.*[\/|\\](.*?)$/) and $filename = $1;
			...
			my ($buffer, $data, $bytesread);
			while ($bytesread=read($file,$buffer,1024))
			{
		        	$data .= $buffer;
			}
			if(-e $uploadpath."$filename" )
			{
				$lines .= "The target file of <strong>'$rootpath$filename'</strong> already exists, As a precaution, it will not be overwritten.<br><br>This does not mean that the upload has failed!";
			}
			else
			{
				open (FH, "> ".$uploadpath."$filename");
				binmode FH;
				print FH $data;
				close(FH);
				if(-e $uploadpath."$filename")
				{
					$lines .= "File uploaded.<br>\n";
					if($uploadpath=~ /$rootpath(.*)/)
					{
						$lines .= "Please click <a href='/$1$filename' target='_blank'>here</a> to view it.";
					}
				}
				else
				{
					$lines .= "File was <strong>NOT</strong> uploaded.";
				}
			}
		}	
		else
		{
			$lines .= "File not specified. Nothing uploaded.";
		}
	}
	else
	{
	...
	$lines .= <<HTMLCODE;
	 <FORM ACTION="$scripturl" METHOD="post" ENCTYPE="multipart/form-data">
	 Local file to Upload: <input type="file" name="file">
	 <br><br>
	 Path to upload to : <input type="text" size=75 name="path" value="$rootpath">
	 <br><br>
	 <input type="hidden" name="action" value="upload">
	 <input type="hidden" name="key" value="$key">
	 <input type="hidden" name="cmd" value="upl">
...
HTMLCODE
	...
	{
		$lines .= <<HTMLCODE;
		Use this utility to send a test mail transport using one of the components listed below<BR>
		<BR>
...
sub mysql_test
{
	...
	<B>Connection to a MySQL Database</B><BR>
	";
	...
	<input type="submit" value="Run Query" name="submit"></form>
	</TD></TR></TABLE>
HTMLCODE
	 my $html = html_source();
	 $html =~ s/##content##/$lines/g;
...
HTMLCODE
	...
	elsif(-T $path)
	{
		...
		<HR>
		<TEXTAREA NAME="text" ROWS="20" COLS="82" WRAP="virtual">$text</TEXTAREA>
		</TD></TR></FORM></TABLE>
...
HTMLCODE
	...
	else
	{
		if($path =~ /.*\.(jpg|gif|png|jpeg)$/i)
		{
			...
			output("image/$ext", read_image($path));
			return;
		}
		elsif( -T $path )
		{
			...
			<HR>
			<textarea cols=82 rows=40>$text</textarea>
			</TD></TR></TABLE>
			</TD></TR></TABLE>
HTMLCODE
		}
...
sub documentation
{
		...
		<TD VALIGN="TOP" BGCOLOR="#EEEEEE">
                <B>Copyright</b><br><br>
                Willow was written by Angelstorm Innovations ( <a href="http://angelstorm.co.uk/willow.html">http://angelstorm.co.uk/willow.html</a> )<BR>
                <I>
                <HR><BR>
                This program is free software; you can redistribute it and/or modify 
                it under the terms of the GNU General Public License as published by
...
sub server_info
{
        ...
        $lines .= "<TABLE CELLSPACING=2 CELLPADDING=2 WIDTH=600>\n";
        my $bgcol=1;
        my $perlv = $];
	$perlv =~ s/000//g;
	$perlv =~ s/00//g;
		...
		foreach my $package ( sort @packages )
		{
			...
			{
				# If RPM returns nothing, it might have been compiled from source
		...
		my $httpd = find_binary("httpd");
		my $result = `$httpd -l`;
		if ($result =~ /.*Compiled-in modules:\n(.*)/s)
		{ 
			$line = $1;
			$line =~ s/\n/<br>\n/g;
			$bgcol = 1-$bgcol;
			$lines .= "<TR><TD CLASS=bgcolor$bgcol valign=\"top\"><B>Httpd compiled in modules</B></TD><TD CLASS=bgcolor$bgcol>$line</TD></TR>\n";
		}
        }
        $lines .= "</TABLE>";
        my $html = html_source();
...
<TR>
	...
	<TD ALIGN="CENTER" BGCOLOR="#DDDDDD"><B>&nbsp;[Server Utilites]&nbsp;</B></TD>
	<TD ALIGN="CENTER" BGCOLOR="#DDDDDD"><B>&nbsp;[Server Tests]&nbsp;</B></TD>
...
<TR>
	...
	<A HREF="$scripturl&amp;cmd=tim">-System Date/Time-</A><BR>
	</TD>
	<TD VALIGN="TOP" ALIGN="CENTER" BGCOLOR="#EEEEEE">
	<A HREF="$scripturl&amp;cmd=sql">-MySQL Interface-</A><BR>
	<A HREF="$scripturl&amp;cmd=brw">-Directory Browser-</A><BR>
	<A HREF="$scripturl&amp;cmd=psh">-Pseudo Shell-</A><BR>
	</TD>
	<TD VALIGN="TOP" ALIGN="CENTER" BGCOLOR="#EEEEEE">
	<A HREF="$scripturl&amp;cmd=eml">-Email Component Test-</A><BR>
	<A HREF="$scripturl&amp;cmd=upl">-File Upload Test-</A><BR>
	<A HREF="$scripturl&amp;cmd=ftp">-FTP Module Test-</A><BR>
	<A HREF="$scripturl&amp;cmd=htp">-HTTP Get Test-</A><BR>
	<A HREF="$scripturl&amp;cmd=cpu">-CPU Math Performance-</A><BR>
	</TD>
	<TD VALIGN="TOP" ALIGN="CENTER" BGCOLOR="#EEEEEE">
...
sub style_sheet
{
 ...
 .system {font-family: Courier; font-size:10px; color: #FFFFFF; font-weight: normal; line-height:12px; border: 0px solid; background: #000000;}
...
sub read_image
{
        my $file = shift or die "Must supply argument to read_image";
        my $line;
        my $buff;
...
sub pass_error
{
	...
	$lines .= "<TABLE CELLSPACING=2 CELLPADDING=2 WIDTH=600><TR><TD VALIGN=\"TOP\" BGCOLOR=\"#EEEEEE\">\n";
...
sub bad_password
{
	...
	my @badpass = ("password", "qwerty", "letmein", "willow", "test", "secure", "pass", "1234");
	foreach(@badpass)
	{
		($password =~ /$_/i) and return 1;
	}
}
